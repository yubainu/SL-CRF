...
ONTOS detects this — instantly, without reading a single word.
---
...
<img width="4200" height="3000" alt="Fig1 Numerical “Ghost Drift” Test" src="https://github.com/user-attachments/assets/4c2c647e-fed3-4b2b-b729-b967e7c4b7a5" />
...
<img width="1536" height="757" alt="Fig2 Textual Hallucination Test" src="https://github.com/user-attachments/assets/5b99096d-a820-4a16-b1ee-f48d5f8cc8d6" />
...
A: It depends on the specific model and task. We recommend running demo_numeric.py to establish a baseline and setting the initial threshold at approximately +2σ (standard deviations) from the mean of normal operation.
...
Patent Pending. The core logic of the Internal Dissonance Index (IDI) and structural monitoring methods implemented in ONTOS are currently under patent application.
...
### クイックスタート
以下の手順で、ローカル環境にてONTOSの構造監視を検証できます。
1 リポジトリをクローンする git clone https://github.com/yubainu/SL-CRF.git cd SL-CRF  
2 数値異常検知の実行 python demo_numeric.py  
3 ハルシネーション検知の実行 python demo_text_hallucination.py  

### 各デモの詳細解説  
#### 1. 数値異常検知 (demo_numeric.py)
「安定した状態」から「崩壊していく状態」への推移をシミュレートします。  
正常フェーズ: データは正常範囲内で推移し、IDI（不協和指数）は0付近で安定します。  
崩壊フェーズ: データが漂流（ドリフト）を始めると、数値が極端な値になる前に、ONTOSが「構造的な摩擦」を検知し、赤色のIDIラインが急上昇します。  
目的: 異常が起きてからではなく、異常に至る「予兆」を捉えられることを証明します。
    
#### 2. ハルシネーション検知 (demo_text_hallucination.py)
テキスト生成中のAI内部メトリクスをシミュレートします。  
正常なテキスト: 整合性の取れた文章では、IDIは低く抑えられます。  
ハルシネーション: AIが「リンゴは重力の味がする」といった支離滅裂な内容を生成した瞬間、内部構造の整合性が破綻します。  
アラート: ONTOSはそのステップを「構造的不連続性」として特定し、信頼性の低い出力をリアルタイムで遮断できることを示します。
...
### FAQ
#### Q: 既存のハルシネーション検知（セマンティック・チェック等）と何が違うのか？  
A: 従来の検知は「意味（セマンティクス）」を検証しますが、ONTOSは「構造（トポロジー）」を監視します。AIが言葉を生成するプロセスの不協和音（Internal Dissonance）を数値化するため、辞書や検索エンジンを参照することなく、生成中の「兆候」を即座に特定できます。

#### Q: なぜ「言葉を一切読まずに」検知できるのか？  
A: ONTOSはテキストの内容そのものではなく、モデルの内部で計算される確率分布や注意（Attention）の重みの「歪み」をデータとして受け取るからです。家の構造計算と同じで、壁の色を見るのではなく、梁のたわみを見て崩壊を予見するアプローチです。

#### Q: IDI（不協和指数）の閾値はどう設定すべきか？  
A: 使用するモデルやタスクによって異なります。まずは demo_numeric.py を使用してベースラインとなる数値を測定し、通常時の平均値から+2σ（標準偏差の2倍）程度を目安に設定することを推奨します。

#### Q: 計算負荷は高いのか？  
A: 極めて軽量です。大規模言語モデル（LLM）自体の計算量に比べれば、ONTOSの構造解析に必要な計算リソースは無視できるほど小さく、リアルタイム監視に適しています。

## コンタクト  
ご興味のある方は、以下のアドレスにお問い合わせください。  
yubainu98@gmail.com
